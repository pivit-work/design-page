 ...
 *
 * 조직장 지정 — `onToggleLeader` (PW-400 · 라벨 PW-422)
 *  · **주입한 호출부에만** `[조직장으로]` 가 뜬다. 미주입이면 버튼 자체가 없다.
 *  · 라벨은 `[매니저로]` 가 아니다. 같은 화면의 스쿼드 팝업에도 리드 지정 버튼이 있는데,
 *    둘 다 「매니저로」 이면 **권한이 오르는 쪽(여기)과 오르지 않는 쪽(스쿼드, SQ11)** 이
 *    같은 말로 보인다. 지정된 뒤의 배지도 같은 이유로 「조직장」 이다.
...
const DEFAULT_LABELS = {
  ...
  // 조직장 지정 — PW-400 (라벨은 PW-422 에서 「매니저로」 → 「조직장으로」)
  makeLeader: '조직장으로',
  makeLeaderTitle: '이 조직의 조직장으로 지정합니다. 권한이 매니저로 오르고 팀당 1명만 유지됩니다',
  leaderBadge: '조직장',
...
export default function OrgTreePicker({
   ...
   *
   * 미주입이면 `[조직장으로]` 버튼이 **아예 뜨지 않는다** — 눌러도 아무 일 없는 버튼을
