...
export default function OrgNode({ node, depth = 0, showWorkHours, showVacation, editMode, adminMode, baseUrl = '', onMemberClick }) {
        ...
        <div className={`members-list ${isDropTarget ? 'drop-target' : ''}`}>
          {node.members.map((m, i) => (
            <React.Fragment key={`${node.id}_${m.name}_${i}`}>
              ...
              <MemberCard member={m} parentId={node.id} index={i} showWorkHours={showWorkHours} showVacation={showVacation} editMode={editMode} adminMode={adminMode} baseUrl={baseUrl} onMemberClick={onMemberClick} />
            </React.Fragment>
          ))}
        ...
        <div className="children-row">
          {node.children.map(child => (
            <div key={child.id} className="child-branch">
              <OrgNode node={child} depth={depth + 1} showWorkHours={showWorkHours} showVacation={showVacation} editMode={editMode} adminMode={adminMode} baseUrl={baseUrl} onMemberClick={onMemberClick} />
            </div>
          ))}
        </div>
      )}
