 ...
 */
function cellHint(L, isMine, editable) {
  if (isMine) return L('squad.tip.clickMyCapacity');
  // 타인 셀에서 열리는 것은 ①·③ 뿐이다 — 「캐파 편집」 을 약속하면 비활성 슬라이더가
  // 버그로 읽힌다(§5-3.9 2).
  return editable ? L('squad.tip.clickEditShare') : '';
}
 ...
 * 화면이 설명하지 못한다(§10-A19).
 */
function capacityNote(L, { total, diff, count, unsetCount }) {
  if (total === 0) return L(unsetCount > 0 ? 'squad.matrix.noteExcluded' : 'squad.matrix.noteUnassigned');
  if (diff > 0) return L('squad.matrix.noteOver', { diff, count });
...
export default function SquadCanvas({
  ...
  /** 닫기 버튼 문구 — 호스트가 번역해 넘긴다. 안 주면 `labels` 의 `squad.dismiss`. */
  actionErrorDismissLabel: actionErrorDismissLabelProp,
  ...
  showProjectTab = true,
  /**
   * 화면 문구 — 객체(`{ key: 문구 }`)나 함수(`(key, vars) => 문구`)로 번역을 넘긴다(PW-705).
   * 안 넘기면 한국어 기본값이다. 키 목록은 `orgchart-labels.jsx`.
   */
  labels,
}) {
  ...
  const actionErrorDismissLabel = actionErrorDismissLabelProp || L('squad.dismiss');
  const statusText = (status) => squadStatusText(L, status);
  const idleHint = L('squad.capacityIdleHint');
  ...
  const lockReason = useCallback(
    () => L(hasOrgAxis ? 'squad.tip.lockedOrg' : 'squad.tip.lockedLead'),
    [hasOrgAxis, L],
  );
  ...
  const submitSquadForm = async () => {
    ...
    const errs = {};
    if (!name) errs.name = L('squad.form.errNameRequired');
    // 동명 검사 — 수정 시에는 자기 자신을 비교에서 제외한다(서버 409 를 앞당겨 잡는다)
    else if (squads.some((s) => s.id !== f.id && s.name.trim().toLowerCase() === name.toLowerCase())) {
      errs.name = L('squad.form.errNameDuplicate');
    }
    if (f.endDate && f.endDate < f.startDate) errs.endDate = L('squad.form.errEndBeforeStart');
  ...
  const tabStrip = (
    <div className="tab-nav">
      ...
      <span className="tab-active">{L('tab.squad')}</span>
    </div>
  );
      ...
      <div className="content-canvas">
        <div className="pj-header">
          ...
          <div className="header-subtitle">
            ...
            <span className="brand-count">{L('squad.count', { count: squads.length })}</span>
          </div>
        </div>
        ...
        <div className="pj-body">
          ...
          <div className="sq-toolbar">
            <div>
              ...
              {/*
                ...
                있습니다」 가 떴다 — 정작 리드는 조직 밖 사람을 데려오는 중이라 화면이 자기가
                방금 한 일과 반대되는 말을 했다(PW-423).

                🔴 세 문장 모두 ②를 **같은 말로** 끝낸다(v3.13). 자격에 따라 캐파 문장이
              ...
              <p className="sq-toolbar-desc">
                {isEditing
                  ? (editableSet === null
                    ...
                    : hasOrgAxis
                      ? L('squad.desc.editOrg')
                      : L('squad.desc.editLead'))
                  : L('squad.desc.view')}
              </p>
            </div>
            <div className="sq-toolbar-actions">
              {ledgerReady && !squadForm && (
                ...
                >
                  <PlusIcon size={14} /> {L('squad.create')}
                </button>
              )}
              {canEditAssignments && (
                <button
          ...
          {!loading && error && (
            <div className="sq-banner sq-banner-error" data-testid="squad-error-banner">
              <div className="sq-banner-title">{L('squad.loadFailed')}</div>
              {onRetry && (
            ...
            <>
              ...
              {overdueSquads.length > 0 && (
                <div className="sq-banner sq-banner-warn" data-testid="squad-overdue-banner">
                  <div className="sq-banner-title">
                    <WarningIcon size={14} /> {L('squad.overdueTitle', { count: overdueSquads.length })}
                  </div>
                  <div className="sq-banner-body">
                    {overdueSquads.map((sq) => `${sq.name} (${fmtYmd(sq.endDate, L('squad.dateUndecided'))})`).join(' · ')}
                  </div>
              ...
              {squads.length === 0 && !squadForm && (
                <div className="sq-empty" data-testid="squad-empty-state">
                  <p className="sq-empty-title">{L('squad.empty.title')}</p>
                  {ledgerReady ? (
                    <>
                      <p className="sq-empty-desc">{L('squad.empty.descAdmin')}</p>
                      <button
                    ...
                    <p className="sq-empty-desc">{L('squad.empty.descViewer')}</p>
                  )}
                </div>
              )}
                  ...
                  return (
                    ...
                    >
                            ...
                            >
                              ...
                              {sqTransitions.map((t) => (
                                <div
                                  ...
                                  className="sq-menu-item"
                                >
                                  {L(`squad.transition.${t.key}`)} <span className="sq-menu-item-to">→ {statusText(t.to)}</span>
                                </div>
                              ))}
                            </AnchoredLayer>
                          </>
                      ...
                      <div className="sq-period">
                        <CalendarIcon size={14} /> {fmtYmd(sq.startDate, L('squad.dateUndecided'))} – {fmtYmd(sq.endDate, L('squad.dateUndecided'))}
                      </div>
                      ...
                      {!isEditing ? (
                        <div className="sq-members">
                          ...
                          <span className="sq-member-count">
                            ...
                            {lead
                              ? nameOf(lead.userId)
                              : <em className="sq-unset">{L('squad.card.leadUnset')}</em>}
                          </span>
                        </div>
                      ) : (
                        <div>
                          <div className="sq-chip-row">
                            {members.map((mm) => {
                              ...
                              return (
                                <div
                                  ...
                                  title={`${L('squad.tip.memberSummary', { name: nameOf(mm.userId), share: mm.sharePct || 0, capacity: capText(mm) })}${isCapacityIdle(mm) ? `\n${idleHint}` : ''}${editable ? `\n${L('squad.tip.clickEditAll')}` : `\n${lockReason()}`}`}
                                >
                                  {mm.role === 'lead' && (
                            ...
                            {members.length === 0 && (
                              <span className="sq-chip-empty">{L('squad.card.noMembers')}</span>
                            )}
                          </div>
                          ...
                          {!canAddToSquad(sq.id) ? (
                            ...
                            <p className="sq-add-caption">{L('squad.card.addCaption')}</p>
                          ) : (addTarget === sq.id ? (
                            <div className="sq-add-wrap">
                              <input
                                autoFocus value={addQuery} aria-label={L('squad.card.searchMember')}
                              ...
                              <div className="sq-add-list">
                                ...
                                {candidates.length === 0 && (
                                  <div className="sq-add-none">
                                    ...
                                    {L(editableSet !== null && !isLeadOf(sq.id) ? 'squad.card.noCandidatesOutOfScope' : 'squad.card.noCandidates')}
                                  </div>
                                )}
                              </div>
                            </div>
                ...
                <div className="sq-panel">
                  ...
                  {myCapacity && (
                    <div className="sq-mycap" data-testid="squad-my-capacity-banner">
                      <span className="sq-mycap-label">{L('squad.mycap.label')}</span>
                      ...
                      <span className="sq-mycap-note">
                        {myCapacity.diff > 0
                          ? L('squad.mycap.over', { diff: myCapacity.diff })
                          : L('squad.mycap.slack', { diff: -myCapacity.diff })}
                        {myCapacity.unset > 0 && (
                          <span className="sq-mycap-unset">{L('squad.mycap.unset', { count: myCapacity.unset })}</span>
                        )}
                      </span>
                      <span className="sq-mycap-owner">
                        {rich(L('squad.mycap.owner'))}
                      </span>
                      {myCapacity.target && (
                        <button
                          type="button"
                        ...
                        >
                          {L(myCapacity.unset > 0 ? 'squad.mycap.setCta' : 'squad.mycap.adjustCta')} →
                        </button>
                      )}
                    </div>
                  )}
                  ...
                  <div className="sq-legend">
                    ...
                    <span className="sq-legend-item">
                      {rich(L('squad.legend.rows'))}
                    </span>
                    <span className="sq-legend-item">
                      {L('squad.legend.capacityPrefix')}<span className="sq-legend-over">{L('squad.legend.capacityOver')}</span>
                      {L('squad.legend.capacityRest')}
                    </span>
                    <span className="sq-legend-item">
                      {rich(L('squad.legend.owner'))}
                    </span>
