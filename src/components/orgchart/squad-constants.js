 ...
 */
/**
 * 팝오버 폭 — **디자인 시안이 아직 없는 값**이다(PW-476).
 *
 * 종전 272 는 근거 없이 고른 숫자였고, 그 폭에서는 「① 스쿼드 내 비중 · 이 스쿼드 100
 * 기준 · 스쿼드 리드가 정하는 값」 같은 **한 줄짜리 캡션이 두 줄로 접혔다.** 접힌 만큼
 * 본문이 길어져 아래쪽 슬라이더가 한 화면에 안 들어왔다.
 *
 * 360 은 브라우저 실측에서 나온 값이다 — 접히면 안 되는 캡션 중 가장 긴 것이 300.4px
 * 이고, 본문 글 폭은 `폭 − 테두리 2 − 좌우 여백 32 − 스크롤바 11` 이라 315px 이 남는다.
 * 헤더 안내문(285.8px)도 여유가 있다.
 *
 * 🔴 이 값은 **개발이 임시로 정한 것**이다(PW-476, 커트 2026-08-24 지시 — 시안을
 * 기다리지 않고 먼저 개발). 디자이너가 폭·여백을 정해 주면 그 값으로 바꾼다.
 */
export const POP_W = 360;
